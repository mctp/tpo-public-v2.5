...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/cords-cnvex/$GCP_NAME /work/job
...
if [ ! -z \$TVAR ]; then
   ...
   mkdir -p /work/job/input/target/\$TVARID
   retry 3 gsutil_fast rsync \$TVAR /work/job/input/target/\$TVARID
fi
if [ ! -z \$GVAR ]; then
    GVARID=\$(basename \$GVAR)
    mkdir -p /work/job/input/genome/\$GVARID
    retry 3 gsutil_fast rsync \$GVAR /work/job/input/genome/\$GVARID
fi
if [ ! -z \$CNVX ]; then
   CNVXID=\$(basename \$CNVX)
   mkdir -p /work/job/input/\$CNVXID
   retry 3 gsutil_fast rsync \$CNVX /work/job/input/\$CNVXID
fi
if [[ \$CNVEX_SETTINGS == gs://* ]]; then
    CNVEX_SETTINGS_FILE=\$(basename \$CNVEX_SETTINGS)
    retry 3 gsutil_fast cp \$CNVEX_SETTINGS /work/job/input/\$CNVEX_SETTINGS_FILE
fi
...
retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/cords-cnvex/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/cords-cnvex/$GCP_NAME
