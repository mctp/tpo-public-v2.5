...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/crisp-align/$GCP_NAME /work/job
...
retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/crisp-align/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/crisp-align/$GCP_NAME
