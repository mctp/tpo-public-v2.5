...
## setup run folder
retry 3 gsutil_fast cp gs://$RUNTIME_WORK_BUCKET/pipe/cords-misc/$GCP_NAME/config.txt /work/job/
source /work/job/config.txt

## import postalign data
PALNID=\$(basename \$PALN)
mkdir -p /work/job/input/\$PALNID
retry 3 gsutil_fast rsync \$PALN /work/job/input/\$PALNID
...
retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/cords-misc/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/cords-misc/$GCP_NAME
