...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/crisp-codac/$GCP_NAME /work/job
...
for i in \"\${ALNA[@]}\"; do
    ...
    retry 3 gsutil_fast rsync \$i /work/job/input/\$ALNID
done
...
docker run --rm=true \
       ...
       -v /test:/test \
       gcr.io/$GCP_PROJECT/tpocrisp:$TPO_BOOT_VER /code/pipe/crisp-codac/crisp-codac_docker.sh

## wait for docker to finish
while [ ! -f /work/job/docker_done ]; do sleep 10; done
echo \$(date) 'Run finished' >> /work/job/gcp_script.log

retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/crisp-codac/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/crisp-codac/$GCP_NAME
