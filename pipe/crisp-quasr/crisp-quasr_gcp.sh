...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/crisp-quasr/$GCP_NAME /work/job
...
docker run --rm=true \
       ...
       -v /test:/test \
       gcr.io/$GCP_PROJECT/tpocrisp:$TPO_BOOT_VER /code/pipe/crisp-quasr/crisp-quasr_docker.sh

## wait for docker to finish
while [ ! -f /work/job/docker_done ]; do sleep 10; done
echo \$(date) 'Run finished' >> /work/job/gcp_script.log

retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/crisp-quasr/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/crisp-quasr/$GCP_NAME
