...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/cords-germline/$GCP_NAME /work/job
source /work/job/config.txt

## import data
ALNID=\$(basename \$ALN)
mkdir -p /work/job/input/\$ALNID
if [ ! -f /work/job/regions.bed ]; then
    retry 3 gsutil_fast rsync \$ALN /work/job/input/\$ALNID
else
...
docker run --rm=true \
       ...
       -v /test:/test \
       gcr.io/$GCP_PROJECT/tpocords:$TPO_BOOT_VER /code/pipe/cords-germline/cords-germline_docker.sh

## wait for docker to finish
while [ ! -f /work/job/docker_done ]; do sleep 10; done
echo \$(date) 'Run finished' >> /work/job/gcp_script.log

retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/cords-germline/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/cords-germline/$GCP_NAME
