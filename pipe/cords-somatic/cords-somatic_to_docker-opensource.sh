...
if [ "$SOMATIC_TINSCOPE" = true ] ; then
    echo "WARNING: It makes no sense to run MuTect2 (tumor-in-normal mode) with only tumor or only normal samples. Even though TINSCOPE is set to be true in the config file, nothing will be output in this mode." >> $docker_script_log
fi
...
if [ "$SOMATIC_TNSCOPE" = true ] ; then
    ...
    echo $(date) "Get pileups" >> $docker_script_log
    parallel "-j${NCORES}" \
    ...
    echo $(date) "Merge pipeup summaries" >> $docker_script_log
    PILEUPS=$(ls -1 $MUT_TMP/$ID-*-pileups.table | sed "s#^#-I #" |tr '\n' ' ')
