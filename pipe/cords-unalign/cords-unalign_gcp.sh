...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/cords-unalign/$GCP_NAME /work/job
source /work/job/config.txt

## import data
retry 3 gsutil_fast cp \$ALN /work/job/input
...
retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/cords-unalign/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/cords-unalign/$GCP_NAME
