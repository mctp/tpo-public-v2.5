...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/crisp-quant/$GCP_NAME /work/job
source /work/job/config.txt

## import data
IFS=';' read -ra FQ1SA <<< \$FQ1S
for i in \"\${FQ1SA[@]}\"; do
    retry 3 gsutil_fast cp \$i /work/job/input
done
...
## setup code
echo \$(date) 'Setting up code' >> /work/job/gcp_script.log
...
echo \$(date) 'Running in docker' >> /work/job/gcp_script.log
docker run --rm=true \
       -e SENTIEON_LICENSE \
       --volumes-from \$CODE \
       -v /tpo:/tpo \
       ...
       -v /test:/test \
       gcr.io/$GCP_PROJECT/tpocrisp:$TPO_BOOT_VER /code/pipe/crisp-quant/crisp-quant_docker.sh

## wait for docker to finish
while [ ! -f /work/job/docker_done ]; do sleep 10; done
echo \$(date) 'Run finished' >> /work/job/gcp_script.log

retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/crisp-quant/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output|docker_done|docker_start' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/crisp-quant/$GCP_NAME
