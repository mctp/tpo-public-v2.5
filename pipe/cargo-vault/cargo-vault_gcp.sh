...
## setup run folder
retry 3 gsutil_fast rsync -r gs://$RUNTIME_WORK_BUCKET/pipe/cargo-vault/$GCP_NAME /work/job
source /work/job/config.txt

## import data
mkdir -p /work/job/input/carat-anno
mkdir -p /work/job/input/cords-misc-tumor
mkdir -p /work/job/input/cords-misc-normal
mkdir -p /work/job/input/cords-cnvex
mkdir -p /work/job/input/crisp-tquasr
mkdir -p /work/job/input/crisp-nquasr
mkdir -p /work/job/input/crisp-codac
if [ ! -z \"\$CARAT_ANNO\" ]; then
   gsutil_fast rsync \$CARAT_ANNO /work/job/input/carat-anno
fi
if [ ! -z \"\$CORDS_MISC_TUMOR\" ]; then
   gsutil_fast rsync \$CORDS_MISC_TUMOR /work/job/input/cords-misc-tumor
fi
if [ ! -z \"\$CORDS_MISC_NORMAL\" ]; then
   gsutil_fast rsync \$CORDS_MISC_NORMAL /work/job/input/cords-misc-normal
fi
if [ ! -z \"\$CORDS_CNVEX\" ]; then
   gsutil_fast rsync \$CORDS_CNVEX /work/job/input/cords-cnvex
fi
if [ ! -z \"\$CRISP_QUASR_TUMOR\" ]; then
   gsutil_fast rsync \$CRISP_QUASR_TUMOR /work/job/input/crisp-tquasr
fi
if [ ! -z \"\$CRISP_QUASR_NORMAL\" ]; then
   gsutil_fast rsync \$CRISP_QUASR_NORMAL /work/job/input/crisp-nquasr
fi
if [ ! -z \"\$CRISP_CODAC\" ]; then
   gsutil_fast rsync \$CRISP_CODAC /work/job/input/crisp-codac
fi
if [[ \$VAULT_SETTINGS == gs://* ]]; then
    VAULT_SETTINGS_FILE=\$(basename \$VAULT_SETTINGS)
    retry 3 gsutil_fast cp \$VAULT_SETTINGS /work/job/input/\$VAULT_SETTINGS_FILE
fi
...
retry 3 gsutil_fast rsync -r /work/job/output gs://$RUNTIME_WORK_BUCKET/repo/cargo-vault/\$ID
...
retry 3 gsutil_fast rsync -x 'input|output' -r /work/job gs://$RUNTIME_WORK_BUCKET/pipe/cargo-vault/$GCP_NAME
