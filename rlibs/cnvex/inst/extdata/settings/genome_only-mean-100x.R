OPTS <- list(
    ...
    ## BAF
    baf.min.dp=12, # pre-filter minimum depth
    baf.max.per.tile=1, # maximum number of SNPs per tile
    baf.max.eff.dp=300, # SNPs with higher depth are not more confident
    baf.het.range.genome=c(NA, NA), # BAF range in normal genome VCF
    baf.het.range.target=c(NA, NA), # BAF range in normal target VCF
    baf.het.min.dp.genome=18, # BAF minimum depth in normal genome VCF (or tumor if tumor-only) to call HETs
    baf.het.min.dp.target=NA, # BAF minimum depth in normal target VCF (or tumor if tumor-only) to call HETs
    baf.cov.min.dp.genome=24, # BAF mimimum depth in tumor (or normal) genome VCF to keep SNP for (tumor or normal) analysis
    baf.cov.min.dp.target=NA, # BAF mimimum depth in tumor (or normal) target VCF to keep SNP for (tumor or normal) analysis
    ...
    prune.nvar=FALSE,
    prune.lr.nvar.penalty=2.5,
    prune.baf.nvar.penalty=2.0,
    prune.hq=TRUE,
    prune.lr.hq.penalty=2.5,
    ...
    bias.gc.adjust.span.off=NA_real_,
    ...
    ## bias.reptime.adjust.span.off=NA_real_,
    ...
    ## bias.meth.adjust.span.off=NA_real_,
    ...
    pool.sd.out=3,
    pool.n.comp=20,
    pool.filter=FALSE, # If TRUE tiles only hq, <hi.nvar, <hi.zero tile will be used in SVD
    
    ## llik
    opt.max.C=14,
    opt.max.sC=36,
    opt.max.len.per.probe=1e6,
    opt.max.snp.per.segment=300,
    opt.p.af.anom=0.001,
    opt.p.lr.anom=0.01,
    opt.dp.af.max=300,
