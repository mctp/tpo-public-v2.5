            ...
            (   ## SNVs and indels outside long STRs and RMSKs
              ...
              (   ## indels inside long STRs and RMSKs
                ((str & str_len>=4) | (is.indel & lengths(rmsk_hit)!=0)) &
                  ...
                  (tlod>24 & nlod>24 & (flod>12 | (ADN==0 & DPN>=120)) & xfet<5e-3 & ADT_FWD>=9 & ADT_REV>=9 & ADN<=1 & AFT>AFN*8)
              ) |
              ## default filters
              (   ## SNVs and indels outside long STRs and RMSKs
                ((!str | str_len<4) & (!is.indel | lengths(rmsk_hit)==0)) &
...
.somatic.filter.anno.tn.fun <- function(rep, GOI) {
  fa <- rep[,
            (
              ...
              (cosmic_cnt > 25 | SYMBOL %in% GOI | clinvar %in% UNLIKELY_BENIGN) &
                ...
                (
                  (
                    ...
                    ((!str | str_len<4) & (!is.indel | lengths(rmsk_hit)==0)) &
                      ...
                      (sblr<2.0 & sor<3.5 & (!is.na(mqrs) & mqrs>-2.5) & mqs>26.0 & ecnt<5)
                  ) |
                    (
                      ...
                      ((str & str_len>=4) | (is.indel & lengths(rmsk_hit)!=0)) &
                        ...
                        (sblr<1.5 & sor<3.0 & (!is.na(mqrs) & mqrs>-2.0) & mqs>26.0 & ecnt<3)
                    )
                )
            )
  ]
...
#' somatic filter of annotation data for Tumor only
#'
#' @param rep - table to filter
#' @param GOI genes of interest
#' @return T/F values regarding pass
...
.somatic.filter.anno.to.fun <- function(rep, GOI) {
  fa <- rep[,
            (   ## important genes / variants
              (clinvar %in% UNLIKELY_BENIGN | cosmic_cnt > 25 | SYMBOL %in% GOI) &
                ## SNVs and indels outside long STRs and RMSKs
                ((!str | str_len<5) & (!is.indel | lengths(rmsk_hit)==0)) &
                ...
                (!is.na(mqrs) & mqrs>-0.5) &
                mqs>26 &
                (gnomad_af==0 & kg_af == 0 & lengths(pon) == 0)
            )
  ]
  return(fa)
}
...
somatic.filter.fun <- function(rep, GOI=NULL) {
  ## goi
  if( is.null(GOI) ) { GOI <- readLines(system.file("extdata/goi/goi-somatic-tier1.txt", package="cargo")) }
  ## modify
  rep %<>%
    dplyr::mutate(is.indel = str_length(ref)!=str_length(alt)) %>%
    dplyr::mutate(sblr = abs(log2((ADT_FWD+1)/(ADT_REV+1))) ) %>%
  ...
  somatic <- rep %>%
    ...
    dplyr::select(-c(is.indel,sblr,all.pon))
  return(somatic)
}
...
germline.filter.fun <- function(rep, GOI=NULL) {
  ...
  ## filter goi
  fgoi <- germline.filter.goi(rep, GOI)
  ## return
  germline <- rep %>%
...
#' @return T/F values regarding pass
#'
germline.filter.goi <- function(rep, GOI=NULL){
