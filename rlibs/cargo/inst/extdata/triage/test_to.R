...
triage.variant.consequence.fun <- function(rep, anno) {
  tri_consequence <- rep |>
    ...
    mutate(!(IMPACT %in% c("HIGH", "MODERATE"))) |> 
    pull()
...
triage.fusion.evidence.fun <- function(rep, anno) {
  tri_evidence <- rep |> 
    mutate(!((hq.bpt & (mm2.valid|gmap.valid)) | hi.bpt)) |> 
    pull()
  return(tri_evidence)
}
...
triage.structural.evidence.fun <- function(rep, anno) {
  tri_evidence <- rep |> mutate(is.na(MANTA.FILTER)) |> pull()
  return(tri_evidence)
}
    ...
    mutate( tmp =
              (   ## SNVs and indels outside long STRs and RMSKs
                ((!str | str_len<4) & (!is.indel | is.na(rmsk_hit))) &
                  ...
                  ((is.na(mqrs) & AFT>0.9) | (!is.na(mqrs) & mqrs>-3)) &
                  mqs>25 & 
                  ((is.na(dbsnp) & gnomad_af<0.01) | (!is.na(dbsnp) & gnomad_af<0.001))
              ) |
              (
                ## indels inside long STRs and RMSKs
                ((str & str_len>=4) | (is.indel & !is.na(rmsk_hit))) &
                  ...
                  ADT_REV>2 &
                  sor<3.5 &
                  ((is.na(mqrs) & AFT>0.9) | (!is.na(mqrs) & mqrs>-2.5)) &
                  mqs>27 & 
                  gnomad_af<0.01 
              )
    ) |>
