...
.runInchworm <- function(bpt, jnc, ann, par) {
    ...
    if (err==0 && length(out)>0) {
        ctg <- data.table(
            ...
            ctg.len = lengths(out))
        ctg <- ctg[(ctg.cov >= par$asm.ctg.cov) | (ctg.len >= par$asm.ctg.len)]
    } else {
        ctg <- CTG.EMPTY
    }
    unlink(ofn)
...
.assembleBreakpoint <- function(bun, ann, par) {
    ctg <- .runInchworm(bun$bpt, bun$jnc, ann, par)
    if (nrow(ctg)>0) {
...
#' @export
assembleBreakpoints <- function(bun, ann, par) {
    buns <- splitBundle(bun)
    lenbig <- sapply(buns, function(bun) nrow(bun$jnc) > 1)
    if (length(lenbig)>0) {
        buns <- buns[lenbig]
        ctg <- rbindlist(mclapply(names(buns), function(idx) {
            sel.bun <- buns[[idx]]
            sel.ctg <- .assembleBreakpoint(sel.bun, ann, par)
            return(sel.ctg)
        }))
        if (nrow(ctg)>0) {
