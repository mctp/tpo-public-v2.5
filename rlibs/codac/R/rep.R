...
.reportSV <- function(bun, spl, ann, par) {
    bpt <- copy(bun$bpt)
    ctg <- copy(bun$ctg)
    ## total reads
    ...
    bpt[, ts.warn:=(unq.rec.5 >= par$bpt.rec.ts.warn | unq.rec.3 >= par$bpt.rec.ts.warn)]
    ## ctg sequence
...
.reportMini <- function(bun, spl, ann, par, only.d2a) {
    bpt <- copy(bun$bpt)
    if (only.d2a) {
        bpt <- bpt[(d2a)]
    }
