.makeFeatures <- function(gtf.fn, par, seqi) {
    txdb <- suppressMessages(suppressWarnings(makeTxDbFromGFF(gtf.fn)))
...
.makeGenes <- function(gtf.fn, par, seqi) {
    tmp <- fread(paste(system2("grep", sprintf("'\tgene\t' %s", gtf.fn), stdout=TRUE),
    ...
    gen <- with(tmp, GRanges(V1, IRanges(V4, V5), V7,
                      ...
                      gene_name = fread(paste(str_match(V9, "gene_name[^;]+"),
                        collapse="\n"), showProgress=FALSE, header=FALSE)$V2,
                      gene_type = fread(paste(str_match(V9, "gene_biotype[^;]+"),
                        collapse="\n"), showProgress=FALSE, header=FALSE)$V2
                      ))
...
.makeTranscripts <- function(gtf.fn, par, fts, seqi) {
    tmp <- fread(paste(system2("grep", sprintf("'\ttranscript\t' %s", gtf.fn), stdout=TRUE),
                       collapse="\n"), showProgress=FALSE, header=FALSE)
    tags <- str_match(tmp$V9, "tags[^;]+")
    tags[is.na(tags)] <- "tags \"none\""
    tx <- with(tmp, GRanges(V1, IRanges(V4, V5), V7,
                      gene_id = fread(paste(str_match(V9, "gene_id[^;]+"),
                        collapse="\n"), showProgress=FALSE, header=FALSE)$V2,
                      transcript_id = fread(paste(str_match(V9, "transcript_id[^;]+"),
                        collapse="\n"), showProgress=FALSE, header=FALSE)$V2,
                      tags = fread(paste(str_match(tags, "tags[^;]+"),
                        collapse="\n"), showProgress=FALSE, header=FALSE)$V2
                      ))
...
.makeCoreAnnotations <- function(gtf.fn, genome, par) {
    ##### seqi,features,genes,transcripts,loci,frames,cytobands
    ## SEQI
    seqi <- keepStandardChromosomes(Seqinfo(genome=genome))
    ## FEATURES
    fts <- .makeFeatures(gtf.fn, par, seqi)
    ## GENES
    genes <- .makeGenes(gtf.fn, par, seqi)
    transcripts <- .makeTranscripts(gtf.fn, par, fts, seqi)
    ## FRAMES
...
#' @export
makeAnnotations <- function(gtf.fn, genome) {
    par <- .findAnnotationFiles(genome)
    core <- .makeCoreAnnotations(gtf.fn, genome, par)
    over <- .makeOverlapAnnotations(core)
    mask <- .makeMaskAnnotations(par, core)
    misc <- .makeMiscAnnotations(par, core)
    ann <- c(core, over, mask, misc)
    return(ann)
}
