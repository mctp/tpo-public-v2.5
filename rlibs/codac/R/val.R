...
#' @export
validateBreakpoints <- function(bun, ann, par) {
    val.mm2 <- .breakpointValidate(bun$aln.mm2, par$asm.mm2.maxdst, par$asm.mm2.minmapq)
    setnames(val.mm2, "x.val", "mm2.val")
    setkeyv(val.mm2, BPT.KEY)
    val.gmap <- .breakpointValidate(bun$aln.gmap, par$asm.gmap.maxdst, par$asm.gmap.minmapq)
